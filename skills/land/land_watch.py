...
AGENT_REPLY_PREFIXES = ("[agent]", "[codex]", "[claude]")
AUTOMATED_REVIEW_HEADINGS = (
    "## Automated Review",
    "## Claude Review",
    "## Codex Review",
)
...
def latest_review_request_at(comments: list[dict[str, Any]]) -> datetime | None:
    ...
    for comment in comments:
        if is_automated_review_bot(comment.get("user", {})):
            continue
        body = comment.get("body") or ""
        if not re.search(r"@(codex|claude)\s+review\b", body, re.IGNORECASE):
            continue
...
def filter_automated_review_comments(
    ...
    automated_comments = [
        c for c in comments if is_automated_review_bot(c.get("user", {}))
    ]
...
def is_automated_review_bot(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    normalized = login.lower()
    return (
        login in AUTOMATED_REVIEW_BOTS
...
def is_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    if is_automated_review_bot(user):
        return True
...
def is_automated_review_body(body: str) -> bool:
    return body.startswith(AUTOMATED_REVIEW_HEADINGS)
...
def filter_human_issue_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for comment in comments:
        ...
        if is_agent_reply_body(body):
            continue
        if is_automated_review_body(body):
            continue
        if re.search(r"@(codex|claude)\s+review\b", body, re.IGNORECASE):
            continue
        created_time = comment_time(comment)
        if (
...
def filter_automated_review_issue_comments(
    comments: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    latest_ack = latest_agent_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_automated_review_body(body):
            continue
        created_time = comment_time(comment)
        if (
            latest_ack is not None
...
def is_blocking_review(
    ...
    if (
        is_automated_review_bot(review.get("user", {}))
    ...
    if is_automated_review_bot(review.get("user", {})):
...
def raise_on_human_feedback(
    ...
    automated_review_comments = filter_automated_review_issue_comments(issue_comments)
    human_review_comments = filter_human_review_comments(review_comments)
    if human_issue_comments or human_review_comments or automated_review_comments:
...
async def wait_for_reviews(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        ...
        bot_issue_comments = filter_automated_review_comments(
            issue_comments, review_request_at
        )
        bot_review_comments = filter_automated_review_comments(
            review_comments, review_request_at
        )
        ...
        if bot_comments:
            ...
            if body:
                print("Automated review left comments. Address before merge.")
                print(body)
                raise SystemExit(2)
    ...
    while True:
        ...
        if not check_runs:
            ...
            if empty_seconds >= CHECKS_APPEAR_TIMEOUT_SECONDS:
                print(
                    "No checks reported within the wait window; confirm repository "
...
async def watch_pr() -> None:
    ...
    if is_merge_conflicting(pr):
        print(
            "PR has merge conflicts. Update from its base branch and push before "
    ...
    async def head_monitor() -> None:
        while True:
            ...
            if is_merge_conflicting(current):
                print(
                    "PR has merge conflicts. Update from its base branch and push "
                    "before running land_watch again.",
                )
                raise SystemExit(5)
            if current.head_sha != head_sha:
                print("PR head updated; sync locally and restart the watcher")
                raise SystemExit(4)
            await asyncio.sleep(POLL_SECONDS)
