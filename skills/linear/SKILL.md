...
name: linear
description: |
  Use the `linear_graphql` client tool provided by Symphony for raw Linear
  GraphQL operations such as comment editing and upload flows.
---

# Linear GraphQL

Use this skill for raw Linear GraphQL work when the `linear_graphql` tool is
available.

## Primary tool

Use the `linear_graphql` client tool exposed by the current session. Symphony
provides the tool and reuses its configured Linear authentication.
